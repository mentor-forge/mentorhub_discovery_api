...
class TestServerExecution(unittest.TestCase):
    ...
    @patch.dict("os.environ", {"API_PORT": "9999"})
    def test_main_execution_uses_api_port_env(self):
        """Test that __main__ execution uses API_PORT from environment."""
        import os

        api_port = int(os.environ.get("API_PORT", 8397))
        self.assertEqual(api_port, 9999)

    def test_default_discovery_port(self):
        """Test default discovery API port constant."""
        from src.server import DISCOVERY_API_PORT

        self.assertEqual(DISCOVERY_API_PORT, 8397)
